...
.NOTES
          ...
          Version - 0.1.4 - () - Enabled logging by loading the PSDUtility module and unremarking logging lines
                                 Validate the the username is in the format DOMAIN\USERNAME (failures are counted against total allowed tries)
                                 Use the Get-Credential interface for gathering credentials
                                 Validate credentials against the webserver
                                 Limit the number of logon tries to 3, reboot WinPE on 3rd failure
...
# This section checks if the PSDDebug environment variable is set to "YES". If it is, it sets a global PSDDebug variable to true.
if ($TSEnv:PSDDebug -eq "YES") {
    $Global:PSDDebug = $true
}
# If the global PSDDebug variable is true, set verbose preference to "Continue" for detailed output.
if ($PSDDebug -eq $true) {
    $verbosePreference = "Continue"
}

# Import the BitsTransfer module globally and forcefully, but without verbose output.
Import-Module BitsTransfer -Global -Force -Verbose:$False
# Import the PSDUtility forcefully, but without verbose output.
Import-Module PSDUtility -Force -Verbose:$False

# Local variables
# Initialize local variables for deployment root, user, password, and credential with empty values.
$global:psddsDeployRoot = ""
$global:psddsDeployUser = ""
$global:psddsDeployPassword = ""
$global:psddsCredential = ""

# Check if the environment variable 'isAuthenticated' is not set (or null)
if ([String]::IsNullOrEmpty($Env:isAuthenticated)) {
    [Environment]::SetEnvironmentVariable("isAuthenticated", "false", "User")
    [Environment]::SetEnvironmentVariable("isAuthenticated", "false", "Process")
}

# Define the main function Get-PSDConnection for establishing a network connection.
function Get-PSDConnection {
    ...
    # Define a C# class WebRequester to perform web requests and handle authentication.
    $csharpCode = @"
    using System;
    using System.Net;

    public class WebRequester {
        public static int GetStatusCode(string url, string domain, string username, string password) {
            try {
...
"@
    ...
    # Add the C# type and the System.Windows.Forms assembly for showing message boxes.
    Add-Type -TypeDefinition $csharpCode -Language CSharp
    Add-Type -AssemblyName System.Windows.Forms

    # Initialize variables for the maximum number of attempts and the current attempt count.
    $maxAttempts = 3
    $attempt = 0

    # Define a function Test-WebServerAccess to use the C# code for checking web server access.
    function Test-WebServerAccess {
        ...
        try {
            return [WebRequester]::GetStatusCode($webpath, $domain, $username, $password)
        }
        catch {
            if ($_ -is [System.Net.WebException]) {
                $webEx = $_
                if ($webEx.Response -ne $null) {
                    $httpResponse = [System.Net.HttpWebResponse]$webEx.Response
    ...
    # Main connection logic, including user authentication and retries if authentication fails.
    # It prompts for credentials, tests the connection, and sets environment variables based on the authentication result.
    # If authentication fails, it shows a message box and retries up to $maxAttempts times.
    # The function also saves the connection details in global variables.

    # Make sure we can connect to the specified location using different methods depending on the path type (HTTP, UNC, or local path).
    # It uses different approaches for different types of paths and handles credentials accordingly.

    if (($Env:isAuthenticated -ne "true") -and ($tsenv:isAuthenticated -ne "true")) {
        # Main connection logic
        while (($Env:isAuthenticated -ne "true") -and ($attempt -lt $maxAttempts)) {
		
			# Prompt the user for credentials.
			Write-PSDLog -Message "$($MyInvocation.MyCommand.Name): Prompt the user for credentials."
			$cred = Get-Credential -Message "Enter <Domain>\UserID and Password"
			# Exit if the user cancels the credential prompt.
			if ($cred -eq $null) {
				Write-PSDLog -Message "$($MyInvocation.MyCommand.Name): User cancelled the operation. Exiting..."
				Show-MessageBox -Text "User cancelled the operation. Exiting..." -Caption "Cancelled"
				Exit
			}

			# Check if the username is in the correct format.
			if ($cred.UserName -match '^.+\\.+') {
				# If format is correct, return the credential object.
				Write-PSDLog -Message "$($MyInvocation.MyCommand.Name): Format is correct ($($credential.UserName))"
				$networkCred = $cred.GetNetworkCredential()
				$userdomain = $networkCred.Domain
				$username = $networkCred.UserName
				$password = $networkCred.Password
				
				Write-PSDLog -Message "$($MyInvocation.MyCommand.Name): Parsed ID: $userdomain\$username"
				#return $credential
				$authTest = Test-WebServerAccess -webpath "$deployRoot/control/Bootstrap.ini" -domain $userdomain -username $username -password $password
				if ($authTest -eq 200) {
					[Environment]::SetEnvironmentVariable("isAuthenticated", "true", "User")
					[Environment]::SetEnvironmentVariable("isAuthenticated", "true", "Process")
					$tsenv:isAuthenticated = "true"
					$tsenv:UserDomain = $userdomain
					$tsenv:UserID = $username
					$tsenv:UserPassword = $password

					Write-PSDLog -Message "$($MyInvocation.MyCommand.Name): Authentication Successful"
				} elseif ($authTest -eq 401) {
					$attempt++
					[string] $deployRoot
					Show-MessageBox -Text "Incorrect Username or Password" -Caption "Attempt $attempt - Incorrect Credentials"
					#Write-PSDLog -Message "$($MyInvocation.MyCommand.Name): Incorrect Username or Password"
					Write-PSDLog -Message "$($MyInvocation.MyCommand.Name): Incorrect Username ($userdomain/$username) or Password ($password)"
				}
				else
				{
					$attempt++
					Show-MessageBox -Text "Error during authentication ($authTest)" -Caption "Attempt $attempt - Authentication Error"
					Write-PSDLog -Message "$($MyInvocation.MyCommand.Name): Failed to authenticate after $maxAttempts attempts."
				}
				
			} else {
				$attempt++
				Show-MessageBox -Text "Username in incorrect format" -Caption "Attempt $attempt - Incorrect Format"
				# Inform the user about the incorrect format and loop again.
				Write-PSDLog -Message "$($MyInvocation.MyCommand.Name): Username in incorrect format ($($credential.UserName))"
			}
			
        }

        if ($Env:isAuthenticated -ne "true") {
            Show-MessageBox -Text "Failed to authenticate after $maxAttempts attempts." -Caption "Authentication Failure"
			Write-PSDLog -Message "$($MyInvocation.MyCommand.Name): Failed to authenticate after $maxAttempts attempts."
            Exit
            #return $false
        }
    ...
    #Show-MessageBox -Text "Connected successfully with domain: $userdomain, username: $username" -Caption "Attempt $attempt"
    Save-PSDVariables | Out-Null
    # Save values in local variables
    # Required for when this function is called by other functions later.
    # Leaving this out causes the USERDOMAIN to be set EMPTY during image boot
    $global:psddsDeployRoot = $deployRoot
    if ($username -like "*\*") {
        $global:psddsDeployUser = $username
    }
    else {
        $global:psddsDeployUser = "$userdomain\$username"
    }
