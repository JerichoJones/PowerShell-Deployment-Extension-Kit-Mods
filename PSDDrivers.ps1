...
.NOTES
          ...
          Version - 0.0.4 - () - Replace "," with "_" in the $SourceDriverPackagePath to support VMware. WIM names with a "," do not extract.
                                 Added output to the Task Sequence during driver gathering/extraction
...
# Replace "," with "_" in the $SourceDriverPackagePath to support VMware. WIM names with a "," do not extract.
...
# Show step during Task Sequence
Show-PSDActionProgress -Message "Creating $($tsenv:OSVolume):\Drivers folder" -Step "1" -MaxStep "1"
...
Foreach($Zip in $Zips){
	...
	Show-PSDActionProgress -Message "Unpacking $($Zip.FullName)" -Step "1" -MaxStep "1"
...
Foreach($WIM in $WIMs){
    ...
    # Show step during Task Sequence
	Show-PSDActionProgress -Message "Unpacking $($WIM.FullName)" -Step "1" -MaxStep "1"
...
foreach($Driver in $Drivers){
    ...
    # Show step during Task Sequence
	Show-PSDActionProgress -Message "$($Driver.Name) is now in the \Drivers folder" -Step "1" -MaxStep "1"
