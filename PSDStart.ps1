...
Get-Volume | ? {-not [String]::IsNullOrWhiteSpace($_.DriveLetter) } | ? {$_.DriveType -eq 'Fixed'} | ? {$_.DriveLetter -ne 'X'} | ? {Test-Path "$($_.DriveLetter):\_SMSTaskSequence\TSEnv.dat"} | % {
	...
	$server = $tsenv:LOGUSERDOMAIN

	# Define the timeout (5 minutes = 300 seconds)
	$timeoutSeconds = 300

	# Define the interval between each ping attempt in seconds
	$pingIntervalSeconds = 10

	# Calculate the end time by adding the timeout to the current time
	$endTime = (Get-Date).AddSeconds($timeoutSeconds)

	# Flag to check if connection is successful
	$connectionSuccessful = $false

	# Start a loop to test the connection
	do {
		try {
			# Ping the server
			$pingResult = Test-Connection -ComputerName $server -Count 1 -ErrorAction Stop
			# If ping is successful, set the flag to true and break the loop
			$connectionSuccessful = $true
			Write-PSDLog -Message "$($MyInvocation.MyCommand.Name): Connection to $server successful."
			#Show-PSDInfo -Message "Connection to $server successful." -OSDComputername $OSDComputername -Deployroot $global:psddsDeployRoot
			break
		} catch {
			# If ping fails, wait for the specified interval then try again
			Start-Sleep -Seconds $pingIntervalSeconds
		}
	} while ((Get-Date) -lt $endTime)

	# Check the connection result after the loop
	if (-not $connectionSuccessful) {
		Write-PSDLog -Message "$($MyInvocation.MyCommand.Name): Failed to connect to $server within the 5-minute timeout."
		#Show-PSDInfo -Message "Failed to connect to $server within the 5-minute timeout." -Severity Error -OSDComputername $OSDComputername -Deployroot $global:psddsDeployRoot
            ...
            if((Test-Path -Path X:\Deploy\Scripts\PSDPrestart.ps1) -eq $true){
                ...
                switch ($Mode)
                {
                    'ClassicFullScreen' {
                        ...
                        $PSDStartLoader = New-PSDStartLoader -FullScreen

                        # Start by hiding all non functioning buttons
    ...
    if(($tsenv:PSDWizard -eq $null) -or ($tsenv:PSDWizard -eq "") -or ($tsenv:PSDWizard -eq "Native")){
        ...
        switch ($PSDWizard)
        {
            'PSDWizardNew' {
                        ...
                        if($tsenv:OSDComputername -eq $Null -or $tsenv:OSDComputername -eq ""){
                            $tsenv:OSDComputername = $env:COMPUTERNAME
                        }


                        # Set theme
                        if($tsenv:PSDWizardTheme -eq $null -or $tsenv:PSDWizardTheme -eq ""){
                        ...
                        # Start the wizard
                        Write-PSDLog -Message ("$($MyInvocation.MyCommand.Name): Running [Show-PSDWizard -ResourcePath {0} -AsAsyncJob:{1} -Passthru -Debug:{2}]" -f $PSDWizardPath,(!$Global:BootfromWinPE),$PSDDebug)
                        # $result = Show-PSDWizard -ResourcePath $PSDWizardPath -AsAsyncJob:(!$Global:BootfromWinPE) -Passthru -Debug:$PSDDebug
                        $result = Show-PSDWizard -ResourcePath $PSDWizardPath -AsAsyncJob:(!$Global:BootfromWinPE) -Passthru -Debug:$PSDDebug -Theme $PSDWizardTheme
