...
.NOTES
          ...
          Version - 0.0.2 - () - Added support for creating driverpacks based on age of driver import folders

.EXAMPLE
	.\New-MDTDriverPackage.ps1 -psDeploymentFolder E:\PSDProduction -CompressionType WIM -DaysOld 1
	.\New-MDTDriverPackage.ps1 -psDeploymentFolder E:\PSDProduction -CompressionType ZIP -DaysOld 1
#>
...
foreach($Driver in $AllDrivers){
    ...
    # Set the flag to indicate at least one folder will be processed
    $foldersToProcess = $true
...
if (-not $foldersToProcess) {
    Write-PSDInstallLog -Message "No folders were found that meet the criteria. Skipping driver package creation." -LogLevel 2
    # Optionally, exit the script or continue with other tasks
    # Exit
} else {
